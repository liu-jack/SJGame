package SJ.Game.friends
{
	 ...
	 * 
	 */	
	public class CJFriendPlayerItemClickMenu extends TabBar
	{
		/*弹出menu上的名字*/
		public const NAME_BUTTON_LIST:Array = ["CHECK" ,  "PRIVATE_CHAT" , "DELETE"]; 
		/*弹出menu*/
		private var _nameBar:TabBar;
		/*关闭按钮*/
		private var _btnClose:Button;
		
		public function CJFriendPlayerItemClickMenu()
		{
			super();
		}
		...
		private function _channelChangeHandler(e:Event):void
		{
			...
			if(e.target is TabBar && (e.target as TabBar).name == "namebar")
			{
				...
				CJEventDispatcher.o.dispatchEventWith(("friendPlayerItemMenuClicked" + ConstFriend.FRIEND_SHOW_TYPE ) , false , {"menu":tabs.selectedItem.name});
			}
		}
		...
		private function _setTabRender():void
		{
			this._nameBar.tabFactory = function():Button
			{
				...
				btn.width = 52;
				btn.height = 23;
				var format:TextFormat = new TextFormat(ConstTextFormat.FONT_FAMILY_HEITI , 10 , 0x000000 , null , null , null , null , null , TextFormatAlign.CENTER);
		...
		private function _setTabBarData():void
		{
			...
			{
				data.push({label : CJLang("FRIEND_"+NAME_BUTTON_LIST[i]) , name:NAME_BUTTON_LIST[i]});
			}
