package SJ.Game.twlogin
{
	...
	public class CJTwRegLayer extends CJTwBgLayer
	{
		private var content:SLayer;
		public function CJTwRegLayer()
		{
			super();
		}
		...
		private function initContent():void
		{
			...
			btnClose.addEventListener(Event.TRIGGERED,function(e:*):void{
				dispatchEventWith(CJEvent.EVENT_NINESPLAY_CLOSE_REG);
			});
			btnClose.x = 319;
			btnClose.y = -19;
			...
			titleText.text = CJLang("TW_REGISTER");
			titleText.x = 129;
			titleText.y = 23;
			...
			var accountTextInput:TextInput = new TextInput;
			...
			accountTextInput.x = accountInputBg.x + 4;
			...
			passwordTextInput.x = pwdInputBg.x + 4;
			...
			surePasswordTextInput.x = surePwdInputBg.x + 4;
			surePasswordTextInput.y = surePwdInputBg.y + 4;
			...
			{
				if(checkValid(accountTextInput.text,passwordTextInput.text))
				{
		...
		private function checkValid(username:String,password:String):Boolean
		{
			// 检测帐号长度是否在6～12之间
