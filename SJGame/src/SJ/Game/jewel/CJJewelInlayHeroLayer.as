package SJ.Game.jewel
{
	 ...
	 * @date 2013-06-08
	 */	
	public class CJJewelInlayHeroLayer extends SLayer
	{
		/** 翻页按钮宽度 **/
		...
		/**上翻按钮**/
//		private var _btnHeroPre:Button;
		/**下翻按钮**/
//		private var _btnHeroNext:Button;
		/**用户武将信息**/
		...
		public function CJJewelInlayHeroLayer()
		{
			super();
		}
		...
		{
			// 修饰框
			var _bg:Scale9Image = new Scale9Image(new Scale9Textures(SApplication.assets.getTexture("common_wujiangxuanzedi") , new Rectangle(15,15,1,1)));
			_bg.width = width;
			_bg.height = height;
			addChild(_bg);
			...
			_heroPanel.y = CONST_PANEL_OFFSET_Y + CONST_BUTTON_HEIGHT;
			_heroPanel.setRect(width - CONST_PANEL_CUT_WIDTH , 200);
			...
			this._heroPanel.nextButton.y = this.height + CONST_BUTTON_OFFSET_Y;
		...
		private function _doInit():void
		{
			...
			{
				const render:CJJewelInlayHeroHeadItem = new CJJewelInlayHeroHeadItem();
		...
		public function selectHero(heroId:String):void
		{
			...
			for (var i:int; i<arr.length; ++i)
			{
				if (String(arr[i].heroid) == heroId)
				{
					...
					this._heroPanel.selectedIndex = i;
				}
				else
				{
