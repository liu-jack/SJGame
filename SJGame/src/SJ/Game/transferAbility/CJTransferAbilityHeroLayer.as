package SJ.Game.transferAbility
{
	 ...
	 * @author zhengzheng
	 */	
	public class CJTransferAbilityHeroLayer extends SLayer
	{
		/** 翻页按钮宽度 **/
		...
		/** Item间隙 **/
		private const CONST_ITEM_GAP:Number = 3;
		/** 滑动面板X轴偏移 **/
...
//		private var _isInit:Boolean = false;
		...
		public function CJTransferAbilityHeroLayer()
		{
			super();
		}
...
//			this._isInit = true;
		...
		{
			_transferAbilityData.addEventListener(CJDataOfTransferAbility.TRANS_FORMATION_DATA_CHANGED , this._updatePanel);
		}
		...
		override public function dispose():void
		{
			...
			{
				_transferAbilityData.removeEventListener(CJDataOfTransferAbility.TRANS_FORMATION_DATA_CHANGED , this._updatePanel);
			}
		 ...
		 * 
		 */		
		private function _updatePanel(e:Event):void
		{
			//传功成功后，删除武将的id
//			var heroIdOld:String = e.data.heroId;
//			var isAfterTrans:Boolean = e.data.isAfterTrans;
			var posTo:int = e.data.posTo;
			var posFrom:int = e.data.posFrom;
		...
		private function _getUnplacedHeroList():Array
		{
			...
			{
				//过滤所有放置过的武将
				if(!_transferAbilityData.isHeroPlaced(heroData.heroid))
				{
					data = new Object;
		...
		private function _doInit():void
		{
			...
			{
				const render:CJTransferAbilityHeroHeadItem = new CJTransferAbilityHeroHeadItem();
