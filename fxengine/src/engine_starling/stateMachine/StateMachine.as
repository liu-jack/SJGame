package engine_starling.stateMachine
{
	...
	public class StateMachine extends starling.events.EventDispatcher
	{
		...
		*/
		...
		protected function mtrace(... parameters):void
		{
//			trace(parameters);
		}
		...
		**/
		...
		{
			if(stateName in _states) mtrace("[StateMachine]",id,"Overriding existing state " + stateName);
		...
		**/
		public function changeState(stateTo:String):void
		{
			...
			if (!(stateTo in _states)){
				mtrace("[StateMachine]",id,"Cannot make transition: State "+ stateTo +" is not defined");
				return;
			}
			...
			{
				mtrace("[StateMachine]",id,"Transition from " + _state + " to "+ stateTo +" denied");
			...
			mtrace("[StateMachine]",id,"State Changed from " + oldState + " to " + _state);
			if(oldState == _state)
			{
				Assert(false,"same state!");
			}
